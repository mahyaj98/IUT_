...
if SECMODE:
    ...
    print("Client public key ", pubkey)
    s.send(bytes(str(pubkey.n).encode()))
    file_mdf = s.recv(100)
    dec_file_mdf = rsa.decrypt(file_mdf, prikey)
    s.send(b'send file')
    file = s.recv(40000)
    if (hashlib.md5(file).digest() == dec_file_mdf):

        with open("code", "wb") as f:
            f.write(file)
        os.system("./code")
    else:
        print("Mismatch in file and its MD5")
        exit()
